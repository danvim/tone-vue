  ...
  <div>
    <vgl-namespace ref="vglNs">
      <vgl-scene name="scn">
        ...
        <vgl-directional-light color="#fff5e8" position="3 28.5 6" :intensity="1" :cast-shadow="true"/>
        <world-map :map="map"/>
      </vgl-scene>
      <div @mousedown.middle.prevent="isCameraRotating = true" @mouseup.middle.prevent="isCameraRotating = false" @mousemove="onDrag" class="field-camera">
        <vgl-renderer scene="scn" camera="cmr1" class="field-camera" antialias ref="renderer" :shadow-map-enabled="true">
...
<script lang="ts">
  ...
  import {s3, v3, VglNamespace as VglNamespaceX, VglRenderer as VglRendererX} from '@/utils/vglHelpers';
  ...
  export default class GameRenderer extends Vue {
    ...
    private setupConnectionListeners(): void {
      window.protocol.conns.forEach((conn: DataConnection) => conn.on('data', (data) => {
